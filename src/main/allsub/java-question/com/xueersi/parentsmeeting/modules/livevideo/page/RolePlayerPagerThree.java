...
import com.xueersi.parentsmeeting.modules.livevideo.activity.item.RolePlayerOtherItemThree;
import com.xueersi.parentsmeeting.modules.livevideo.activity.item.RolePlayerSelfItem;
import com.xueersi.parentsmeeting.modules.livevideo.activity.item.RolePlayerSelfItemThree;
...
//    private RelativeLayout rlResult;
     ...
     * 角色业务
     */
    private RolePlayerBllThree mRolePlayBll;
    ...
    private  LiveAndBackDebug liveAndBackDebug;//只为记录日志调用方便
    ...
    private boolean isRobot;//true 人机
    /**
     * ture 直播，false 回放
     */
    private boolean mIsLive;

    public RolePlayerPagerThree(Context context, RolePlayerEntity obj, boolean isNewView, RolePlayerBllThree rolePlayerBll,
                                LiveGetInfo liveGetInfo, boolean isRobot, LivePagerBack livePagerBack, VideoQuestionLiveEntity videoQuestionLiveEntity) {
        super(context);
        this.isRobot = isRobot;
        this.mRolePlayBll = rolePlayerBll;
        this.livePagerBack = livePagerBack;
        mLiveGetInfo = liveGetInfo;
        rolePlayerEntity = obj;
        this.videoQuestionLiveEntity = videoQuestionLiveEntity;
        mIsLive = videoQuestionLiveEntity.isLive();
...
//        rlResult = view.findViewById(R.id.rl_live_roleplayer_result_main);
        ...
        view.addOnAttachStateChangeListener(new View.OnAttachStateChangeListener() {
            ...
            public void onViewDetachedFromWindow(View view) {
                ...
                if (rolePlayerEntity != null) {
                    rolePlayerEntity = null;//防止结果页数据错乱，尤其点赞个数
                }
    ...
    public void initData() {
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                if (rolePlayerEntity != null) {
                    List<RolePlayerEntity.RolePlayerHead> rolePlayerHeads = rolePlayerEntity.getLstRoleInfo();
                    List<RolePlayerEntity.RolePlayerMessage> rolePlayerMessages = rolePlayerEntity.getLstRolePlayerMessage();
    ...
    private void roleConfirmPage() {
        try {
            mHeadShowAdapter = new RolePlayerHeadShowAdapter(mContext, rolePlayerEntity.getLstRoleInfo());
            int roleHeadsSize = rolePlayerEntity.getLstRoleInfo().size();
    ...
    private void waitRolePlayer() {
        ...
        if (rolePlayerEntity != null && rolePlayerEntity.getLstRolePlayerMessage().get(0).getRolePlayer().isSelfRole()) {
...
//                }
                ...
                rolePlayerEntity.setCountDownSecond(rolePlayerEntity.getCountDownSecond() - 1);
                tvCountTime.setText(getCountDownTime());
...
//
        ...
        //填充对话内容
        mRolePlayerAdapter = new CommonAdapter<RolePlayerEntity.RolePlayerMessage>(rolePlayerEntity.getLstRolePlayerMessage(),
                2) {
            @Override
            public AdapterItemInterface<RolePlayerEntity.RolePlayerMessage> getItemView(Object type) {
                if ((boolean) type) {
                    //自己朗读的
                    mRolePlayerSelfItem = new RolePlayerSelfItemThree(mContext, mRolePlayBll,mReadHandler);
                    return mRolePlayerSelfItem;
                } else {
                    //他人朗读的
                    mRolePlayerOtherItem = new RolePlayerOtherItemThree(mContext, mRolePlayBll,mReadHandler,isRobot);
...
//                    (mContext, 5));
        ...
        public void handleMessage(Message msg) {
            ...
            try {
                ...
                if (STOP_ROLEPLAY == msg.what) {
                    ...
                    if (tempIndex >= rolePlayerEntity.getLstRolePlayerMessage().size()) {
                        return;
                    }
                    if (tempIndex < 0) {
                        tempIndex = 0;
                    }
                    RolePlayerEntity.RolePlayerMessage upMessage = rolePlayerEntity.getLstRolePlayerMessage().get
                            (tempIndex);
                ...
                if (msg.what == READ_MESSAGE) {
                    ...
                    if (mCurrentReadIndex > 0) {
                        ...
                        if (position == 0) {
                            RolePlayerEntity.RolePlayerMessage upMessage = rolePlayerEntity.getLstRolePlayerMessage().get
                                    (mCurrentReadIndex - 1);
                            if ((mCurrentReadIndex - 1) == rolePlayerEntity.getSelfLastIndex()) {
                                mLogtf.i("handleMessage:isResult="+rolePlayerEntity.isResult());
                                if (!rolePlayerEntity.isResult()){
                                    if (rolePlayerEntity.isNewArts()) {
                            ...
                            if (upMessage.getRolePlayer().isSelfRole()) {
                                ...
                                mRolePlayBll.selfReadEnd(upMessage.getStars(), upMessage.getSpeechScore(), upMessage
                                                .getFluency(), upMessage.getAccuracy(), upMessage.getPosition(), rolePlayerEntity,
                            ...
                            for (int i = position; i >= mCurrentReadIndex - 1; i--) {
                                RolePlayerEntity.RolePlayerMessage tempMessage = rolePlayerEntity.getLstRolePlayerMessage().get(i);
                    ...
                    if (mCurrentReadIndex == (rolePlayerEntity.getLstRolePlayerMessage().size())) {
                        //已经对话完毕
                        endRolePlayer();
                        return;
                    } else {
                    ...
                    //取出当前这条的延时时间
                    RolePlayerEntity.RolePlayerMessage currentMessage = rolePlayerEntity.getLstRolePlayerMessage().get
                            (mCurrentReadIndex);
                    ...
                    speechReadMessage(currentMessage, rolePlayerEntity);
    ...
    private void beginRolePlayer() {
        ...
        if(!isRobot){
            if(mWorkerThread == null){
                Toast.makeText(mContext,"声网线程未初始化",Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            mWorkerThread.joinChannel(null, rolePlayerEntity.getLiveId() + "_" + rolePlayerEntity.getTestId() + "_" + rolePlayerEntity.getTeamId(),
    ...
    public void showResult() {
        ...
        logger.i("显示结果,记录日志");
        //显示结果的时候记录日志
        RolePlayLog.sno7(liveAndBackDebug, rolePlayerEntity, mContext);
...
//        rlResult.setVisibility(View.VISIBLE);
        ...
        List<RolePlayerEntity.RolePlayerHead> lstHead = rolePlayerEntity.getResultRoleList();
        ...
        if (head != null) {
            ...
            if (!mLiveGetInfo.getSmallEnglish()) {
                //初中结果页
                RolePlayResultPager rolePlayResultPager = new RolePlayResultPager(mContext, rolePlayerEntity, group);
                ...
                speechResultEntity.gold = rolePlayerEntity.getGoldCount();
                speechResultEntity.energy = rolePlayerEntity.getEnergy();
                ...
                speechResultEntity.praise = rolePlayerEntity.getPullDZCount();
                ...
                RolePlayerPagerThree.this.resultPager = resultPager;
            }
        }
...
//        }
    ...
    public void showDZ(String roleName) {
        rolePlayerEntity.setPullDZCount(rolePlayerEntity.getPullDZCount() + 1);
    ...
    public void setEntity(RolePlayerEntity entity) {
    ...
    private SpannableString getCountDownTime() {
        ...
        boolean isFu = false;
        if (rolePlayerEntity != null) {
            if (rolePlayerEntity.getCountDownSecond() < 0) {
                isFu = true;
                countTime = Math.abs(rolePlayerEntity.getCountDownSecond());
            } else {
                countTime = rolePlayerEntity.getCountDownSecond();
            }
        }
    ...
    public void stopSpeech(){
        ...
        mReadHandler.removeMessages(GO_SPEECH);
        mReadHandler.removeMessages(READ_MESSAGE);
        if (rolePlayerEntity != null&&!rolePlayerEntity.isResult()&&mRolePlayBll.getRoleEntry()!=null) {
            if (rolePlayerEntity.isNewArts()) {
                mRolePlayBll.requestNewArtsResult();
            } else {
                mRolePlayBll.requestResult();
...
//        }
    ...
    public void relaseCurrentPage() {
        ...
        if (group != null) {
            ...
            speechEvalAction.stopSpeech(RolePlayerPagerThree.this, getBaseVideoQuestionEntity(),
