...
public class AsyncNetworkSocket implements AsyncSocket {
    String TAG = "AsyncNetworkSocket";

    AsyncNetworkSocket() {
        Log.i(TAG, "AsyncNetworkSocket");
    }
...
//            ByteBufferList.obtainArray(buffers, Math.min(Math.max(mToAlloc, 2 << 11), maxAlloc));
            ...
            if (read < 0) {
                Log.i(TAG, "onReadable:read=" + read);
                closeInternal();
                closed = true;
            } else {
                total += read;
...
//                }
    ...
    protected void reportClose(Exception e) {
        if (closeReported)
            return;
        if (e == null) {
            Log.i(TAG, "reportClose");
        } else {
            Log.e(TAG, "reportClose" + e.getMessage());
        }
        closeReported = true;
    ...
    void reportEndPending(Exception e) {
        if (e == null) {
            Log.i(TAG, "reportEndPending");
        } else {
            Log.e(TAG, "reportEndPending" + e.getMessage());
        }
