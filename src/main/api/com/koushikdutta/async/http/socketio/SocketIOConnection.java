...
class SocketIOConnection {
    ...
    private void reportDisconnect(final boolean fromRemote, final Exception ex) {
        if (ex != null) {
        ...
        select(null, new SelectCallback() {
            ...
            public void onSelect(SocketIOClient client) {
                if (client.connected) {
                    ...
                    if (closed != null)
                        closed.onDisconnect(fromRemote, ex);
                } else {
    ...
    private void attach() {
        ...
        transport.setClosedCallback(new CompletedCallback() {
            ...
            public void onCompleted(final Exception ex) {
                transport = null;
                reportDisconnect(false, ex);
            }
        });
        final String[] packets = {"disconnect"
                , "connect"
                , "heartbeat"
                , "message"
                , "json"
                ...
                try {
                    ...
                    if (code != 2) {
                        if (code >= 0 && code < packets.length) {
                            Log.d(TAG, "onStringAvailable:packets=" + packets[code] + ",message=" + message);
                        } else {
                            Log.d(TAG, "onStringAvailable:code=" + code + ",message=" + message);
                        }
                    }
                    switch (code) {
                        case 0:
