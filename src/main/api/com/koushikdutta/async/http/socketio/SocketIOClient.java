...
public class SocketIOClient extends EventEmitter {
    ...
    public void emit(String name, JSONObject jsonObject, Acknowledge acknowledge) {
        final JSONObject event = new JSONObject();
        try {
            event.put("name", name);
            event.put("args", jsonObject);
            emitRaw(5, event.toString(), acknowledge);
        } catch (Exception e) {
        }
    }
    ...
    public void disconnect() {
        ...
        if (disconnectCallback != null) {
            disconnectCallback.onDisconnect(false, null);
        }
    }
