...
public class ExperLiveAction {
    ...
    public void onModeChanged(int mode) {
        if (mode == ExperienceRecordFragmentBase.COURSE_STATE_1) {
            // 课前状态,辅导老师在直播间就播放直播
            ivTeacherNotpresent.setImageResource(R.drawable.live_course_open_late);
        } else if (mode == ExperienceRecordFragmentBase.COURSE_STATE_2) {
            // 课中状态,播放回放视频
            ivTeacherNotpresent.setImageResource(R.drawable.live_course_wait_teacher);
        } else if (mode == ExperienceRecordFragmentBase.COURSE_STATE_3) {
            // 课后状态,辅导老师在直播间就播放直播
            ivTeacherNotpresent.setImageResource(R.drawable.live_course_wait_teacher);
        } else if (mode == ExperienceRecordFragmentBase.COURSE_STATE_4) {
            // 结束状态
        ...
        if (mode == ExperienceRecordFragmentBase.COURSE_STATE_1 || mode == ExperienceRecordFragmentBase.COURSE_STATE_3) {
