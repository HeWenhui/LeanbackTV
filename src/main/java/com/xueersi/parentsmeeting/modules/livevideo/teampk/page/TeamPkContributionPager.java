    ...
    private void buildAnimInfo(TeamPkContribStarLottieEffectInfo effectInfo) {
        ...
        if (starList != null && starList.size() > 0) {
            ...
            for (int i = 0; i < starList.size(); i++) {
                ...
                info.getNameInfo().setValue(TextUtils.isEmpty(starInfo.getRealname())?starInfo.getNickname():starInfo.getRealname());
                // 绑定 能量信息
                ...
                //绑定 旋转背景信息
                //学生自己是贡献之星
                info.getAnimBgInfo().setShow(starInfo.getStuId().equals(UserBll.getInstance().getMyUserInfoEntity().getStuId()));
            }
        }
        AnimInfo info;
