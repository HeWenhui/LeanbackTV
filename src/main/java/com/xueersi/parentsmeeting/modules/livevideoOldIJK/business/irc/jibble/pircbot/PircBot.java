...
public abstract class PircBot implements ReplyConstants {
     ...
     * @throws NickAlreadyInUseException if our nick is already in use on the server.
     */
    public final synchronized String connect(String hostname, int port, String password) throws IOException,
        ...
        if (password != null && !password.equals("")) {
            OutputHandler.sendRawLine(this, bwriter, "PASS " + password);
        }
        String nick = this.getName();

        if (saslUsername != null) {
            OutputHandler.sendRawLine(this, bwriter, "CAP LS");
            OutputHandler.sendRawLine(this, bwriter, "CAP REQ : sasl multi-prefix");
            OutputHandler.sendRawLine(this, bwriter, "CAP END");

            OutputHandler.sendRawLine(this, bwriter, "AUTHENTICATE PLAIN");
            ...
            while (authStringEncoded.length() >= 400) {
                ...
                OutputHandler.sendRawLine(this, bwriter, "AUTHENTICATE " + toSend);
            }

            if (authStringEncoded.length() > 0) {
                OutputHandler.sendRawLine(this, bwriter, "AUTHENTICATE " + authStringEncoded);
            } else {
                OutputHandler.sendRawLine(this, bwriter, "AUTHENTICATE +");
            }
        }
...
//        OutputHandler.sendRawLine(this, bwriter, "NXICK " + nick);
        ...
        // XXX: PircBot Patch - Set nick before loop. otherwise we overwrite it
        // in the loop again and again
        ...
        // XXX: PircBot patch - We are not connected to server if nothing
        // received
        if (line == null) {
        ...
        if (_outputhandler == null) {
            HandlerThread thread = new HandlerThread("OutputThread");
            thread.start();
    ...
    private static void setSNIHost(final SSLSocketFactory factory, final SSLSocket socket, final String hostname) {
        SNIHost.setSNIHost(factory, socket, hostname);
    }
    ...
    public final void sendCTCPCommand(String target, String command) {
        _outputhandler.add("PRIVMSG " + target + " :\u0001" + command + "\u0001");
    }
    ...
    protected void handleLine(String line) throws NickAlreadyInUseException, IOException {
            ...
            if (exclamation > 0 && at > 0 && exclamation < at) {
                ...
                if (tokenizer.hasMoreTokens()) {
                    ...
                    if (code != -1) {
                        ...
                        // XXX: PircBot Patch - Sometimes there are senderinfos
                        // with an ident but no host
                        if (sourceNick.contains("!") && !sourceNick.contains("@")) {
                            String[] chunks = sourceNick.split("!");
                            sourceNick = chunks[0]; // Use the part before the
                            // exclamation mark
                        }
        ...
        if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001")) {
            ...
            if (request.equals("VERSION")) {
                ...
                // TIME request
                this.onPong(sourceNick, sourceLogin, sourceHostname, target, request.substring(5));
    ...
    private final void processServerResponse(int code, String response) {
        ...
        if (code == RPL_LIST) {
            ...
            while (tokenizer.hasMoreTokens()) {
                ...
                // XXX: PircBot Patch - Recognize % as prefix - Often used as
                // "half-operator" prefix
                else if (nick.startsWith("%")) {
                    prefix = "%";
                }
    ...
    protected void onPong(String sourceNick, String sourceLogin, String sourceHostname, String target, String pingValue) {
    }
    ...
    protected Vector<String> privMsg = new Vector<>();
    private OutputHandler _outputhandler = null;
